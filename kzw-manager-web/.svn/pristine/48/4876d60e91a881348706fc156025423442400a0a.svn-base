...
public class StoreController {
	...
	@ResponseBody
	public KZWResult updateStrategy(StrategyVo strategyVo, Long storeId, HttpServletRequest request, Model model) {
	...
	public EasyUIDataGridResult order(HttpServletRequest request) {
		...
		pageDate.getParams().put("orderStatus", "orderStatus");
	...
	@ResponseBody
	public EasyUIDataGridResult historyOrder(HttpServletRequest request,String createtimeStr, String updatetimeStr,
			String order_id, String title, String phone) {
		PageData pageDate = PageDataGenerator.newInstall(request, StoreOrder.class, true);
		...
		if(StringUtils.isNotBlank(createtimeStr)) {
			pageDate.getParams().put("createtime", DataUtil.getDateByString(createtimeStr));
		}
		if(StringUtils.isNotBlank(updatetimeStr)) {
			pageDate.getParams().put("updatetime", DataUtil.getDateByString(updatetimeStr));
		}
		if(StringUtils.isNotBlank(title)) {
			pageDate.getParams().put("title", title);
		}
		if(StringUtils.isNotBlank(phone)) {
			pageDate.getParams().put("phone", phone);
		}
		if(StringUtils.isNotBlank(order_id)) {
			pageDate.getParams().put("order_id", order_id);
		}
		pageDate.getParams().put("orderHistoryStatus", "orderHistoryStatus");
		PageData p = storeOrderService.selectPage(pageDate);
