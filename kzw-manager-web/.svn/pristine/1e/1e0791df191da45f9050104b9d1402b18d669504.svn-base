...
<div>
	...
	<span>&nbsp;商品编码:</span>
	<input type="text" id="barcode" style="border:1px solid #95B8E7;"/>
	<span>&nbsp;商品状态:</span>
...
<table class="easyui-datagrid" id="itemList" title="店铺商品列表" 
    ...
    <thead>
        <tr>
            ...
            <th data-options="field:'buyPrice',width:70,align:'right',formatter:TAOTAO.formatPrice">商品进价</th>
        ...
        handler:function(){
        	...
        	$("#itemEditWindow").window({
        		onLoad :function(){
        			...
        			data.price = price;
        			data.priceView = price;
        			var buyPice = data.buyPrice;
        			buyPice = (buyPice/1000).toFixed(2);
        			
        			data.buyPriceView = buyPice;
        			$("#itemeEditForm").form("load",data);
        			// 加载商品de 描述
        			$.get('/item/desc/'+data.id, function(data){
            			if(data.status == 200){
            				itemEditEditor.html(data.data.itemDesc);
            			}
            		});
        			//加载商品的规格
        			$.get('/item/param/'+data.id, function(_data){
