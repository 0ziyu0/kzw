...
import com.kzw.mapper.TbOrderSecondMapper;
...
public class OrderServiceImpl implements OrderService {
	...
	@Resource
	private  TbOrderSecondMapper orderSecondMapper  ;
	...
	@Override
	public int createSeOrder(TbOrderSecond orderInfo) {
		
		return  orderSecondMapper.insert(orderInfo); 
	}

	@Override
	public List<SeOrderInfo> selectByUserIdforbuyer(Integer userId) {
		
		List<SeOrderInfo> resultBuyer = orderSecondMapper.selectByUserIdforbuyer(userId);
		for (SeOrderInfo seOrderInfo : resultBuyer) {
			seOrderInfo.setTypeId("1");
		}

		List<SeOrderInfo> resultSeller = orderSecondMapper.selectByUserIdforseller(userId);
		for (SeOrderInfo seOrderInfo : resultSeller) {
			seOrderInfo.setTypeId("2");
		}		
		resultBuyer.addAll(resultSeller);		
		return resultBuyer;
