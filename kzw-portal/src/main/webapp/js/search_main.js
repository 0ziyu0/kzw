...
SEARCH.bottom_page_html = function(e, k, j) {
    ...
    if (e >= k) {
        ...
        f += '<a onclick="SEARCH.page(' + g + ', true)" href="javascript:;" class="next" >下一页<b></b></a>'
    }
...
//校园二手分页
SEARCH.bottom_page_html_1 = function(e, k, j) {
	// ${page},${totalPages},''
    if (j) {
        this.page_num = e;
        e = Math.ceil(e / 2);
    ...
    if (e <= 1) {
        ...
        f += '<a onclick="SEARCH.page_1(' + d + ', true)" href="javascript:;" class="prev" title="使用方向键左键也可翻到上一页哦！">上一页<b></b></a>'
    }
    for (var h = 1; h <= k; h++) {
        if (h <= 2 || h >= a && h <= b) {
            f += h == e ? '<a href="javascript:;" class="current">' + h + "</a>" : '<a onclick="SEARCH.page_1(' + (j ? 2 * h - 1 : h) + ', true)" href="javascript:;">' + h + "</a>"
        } else {
            if (h < a) {
                f += '<span class="text">…</span>';
                h = a - 1
    ...
    if (e >= k) {
        ...
        f += '<a onclick="SEARCH.page_1(' + g + ', true)" href="javascript:;" class="next" >下一页<b></b></a>'
    }
    f += '<span class="page-skip"><em>&nbsp;&nbsp;共' + k + '页&nbsp;&nbsp;&nbsp;&nbsp;到第</em><input class="jumpto" type="text" value="' + e + '" onkeydown="javascript:if(event.keyCode==13){SEARCH.page_jump_1(' + k + "," + j + ');return false;}"/><em>页</em><a class="btn-skipsearch" value="确定" onclick="SEARCH.page_jump_1(' + k + "," + j + ')" href="javascript:;">确定</a></span>';
    $("#pagin-btm").html(f)
};
//校园二手
SEARCH.page_1 = function(b, a) {
    b = parseInt(b, 10);
    if (b < 1) {
        b = 1
    }
    window.location = "http://localhost:8080/searchSeItem.html?q="+SEARCH.query+"&pageNum="+b;
//    if (a) {
...
SEARCH.bottom_page_html_products = function(e, k, j, id) {
	// ${page},${totalPages},''
    if (j) {
        this.page_num = e;
        e = Math.ceil(e / 2);
    ...
    if (e <= 1) {
        ...
        f += '<a onclick="SEARCH.page_products(' + d + ',' + id + ')" href="javascript:;" class="prev" title="使用方向键左键也可翻到上一页哦！">上一页<b></b></a>'
    }
    for (var h = 1; h <= k; h++) {
        if (h <= 2 || h >= a && h <= b) {
            f += h == e ? '<a href="javascript:;" class="current">' + h + "</a>" : '<a onclick="SEARCH.page_products(' + (j ? 2 * h - 1 : h) + ',' + id + ')" href="javascript:;">' + h + "</a>"
        } else {
            if (h < a) {
                f += '<span class="text">…</span>';
                h = a - 1
    ...
    if (e >= k) {
        ...
        f += '<a onclick="SEARCH.page_products(' + g + ',' + id + ')" href="javascript:;" class="next" title="使用方向键右键也可翻到下一页哦！">下一页<b></b></a>'
    }
    f += '<span class="page-skip"><em>&nbsp;&nbsp;共' + k + '页&nbsp;&nbsp;&nbsp;&nbsp;到第</em><input class="jumpto" type="text" value="' + e + '" onkeydown="javascript:if(event.keyCode==13){SEARCH.page_jump_products(' + k + "," + j + "," + id + ');return false;}"/><em>页</em><a class="btn-skipsearch" value="确定" onclick="SEARCH.page_jump_products(' + k + "," + j + "," + id + ')" href="javascript:;">确定</a></span>';
    $("#pagin-btm").html(f)
};
SEARCH.page_jump_products = function(b, a, id) {
    ...
    if (c > b) {
     ...
     return this.page_products(c, id);
    //return a ? this.page_products(2 * c - 1, id) : this.page(c, id), false
};
SEARCH.page_products = function(b, a) {
    b = parseInt(b, 10);
    if (b < 1) {
        b = 1
        
    }
    window.location = "http://localhost:8080/products/"+a + ".html" + "?page="+b;
};
...
SEARCH.page_jump = function(b, a) {
    ...
    // return this.page(c, true);
    return a ? this.page(2 * c - 1, true) : this.page(c, true), false
};

SEARCH.page_jump_1 = function(b, a) {
    var c = parseInt($("#pagin-btm .jumpto").val(), 10);
    if (isNaN(c)) {
        c = 1
    }
    ...
    // return this.page(c, true);
    return a ? this.page_1(2 * c - 1, true) : this.page_1(c, true), false
};
